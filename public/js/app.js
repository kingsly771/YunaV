...
function initFirebase() {
  ...
  try { if(!firebase.apps.length){ firebase.initializeApp(cfg); } State.firebaseAuth=firebase.auth(); return true; }
...
function initAuth() {
  ...
  State.firebaseAuth.getRedirectResult().then(async result => {
    ...
    try {
      ...
      catch(err) { if(err.data?.banned){ showBannedScreen(err.data.ban); return; } throw err; }
      State.currentUser=data.user;
      if(data.warnings?.length) pendingWarnings=data.warnings;
      if(data.isNewUser){ showAuthStep('profile'); populateProfileStep(data.user); }
      else { enterApp(); }
    } catch(err) {
  ...
  $('#btn-google-signin').addEventListener('click', async () => {
    ...
    try {
      ...
      // Always use redirect — popups are blocked by third-party cookie restrictions
