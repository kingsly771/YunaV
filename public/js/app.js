...
function initAuth() {
  ...
  // Check for auth errors passed back from server
  const params = new URLSearchParams(window.location.search);
  if (params.get('auth_error')) {
    setAuthError('Sign-in failed: ' + params.get('auth_error'));
    window.history.replaceState({}, '', '/');
  }
  if (params.get('banned')) {
    showBannedScreen({ type: params.get('type'), reason: params.get('reason') });
    return;
  }

  // Simple redirect to server-side Google OAuth — no Firebase SDK needed
