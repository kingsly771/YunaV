...
function initFirebase() {
  return true; // replaced by server-side Google OAuth
}
...
function initAuth() {
  ...
  ['auth-reg-name','auth-reg-email','auth-reg-password'].forEach(id => {
  ...
  $('#btn-login').addEventListener('click', async () => {
    ...
    const btn = $('#btn-login'); setBtnLoading(btn, true); setAuthError('');
    try {
  ...
  $('#btn-register').addEventListener('click', async () => {
    ...
    const btn = $('#btn-register'); setBtnLoading(btn, true); setAuthError('');
    try {
...
async function init(){
  try{
    ...
    if(data.user?.isNewUser){ initAuth(); showAuthStep('profile'); populateProfileStep(data.user); }
    else { enterApp(); }
  }catch(err){
    if(err.data?.banned){ showBannedScreen(err.data.ban); return; }
    initAuth();
