...
const upload = multer({
  storage: multer.diskStorage({
    ...
    filename: (req, file, cb) => cb(null, `avatar_${req.session.userId}_${Date.now()}${path.extname(file.originalname)||'.jpg'}`)
  }),
...
router.get('/users/search', requireAuth, (req, res) => {
  ...
  const users = getDB().prepare(
    `SELECT id, email, name, avatar, status, is_online, last_seen
     FROM users WHERE id != ? AND (email LIKE ? OR name LIKE ?) LIMIT 12`
...
router.patch('/profile', requireAuth, (req, res) => {
  ...
  res.json({ user: db.prepare(`SELECT id, email, name, avatar, status FROM users WHERE id = ?`).get(req.session.userId) });
});
...
router.post('/profile/avatar', requireAuth, upload.single('avatar'), (req, res) => {
  ...
  getDB().prepare(`UPDATE users SET avatar = ? WHERE id = ?`).run(url, req.session.userId);
  res.json({ avatar: url });
});
...
router.get('/conversations', requireAuth, (req, res) => {
  ...
  const convs = getDB().prepare(`
    SELECT
      ...
      u.status AS other_status, u.is_online AS other_online,
      u.last_seen AS other_last_seen, u.email AS other_email,
...
router.get('/conversations/:id/messages', requireAuth, (req, res) => {
  ...
  let sql    = `SELECT m.*, u.name AS sender_name, u.avatar AS sender_avatar
                FROM messages m JOIN users u ON u.id = m.sender_id WHERE m.conv_id = ?`;
  ...
  db.prepare(
    `UPDATE messages SET read_at = strftime('%s','now') WHERE conv_id = ? AND sender_id != ? AND read_at IS NULL`
...
router.post('/conversations/start', requireAuth, (req, res) => {
  ...
  if (!targetUserId || targetUserId === uid) return res.status(400).json({ error: 'Invalid target' });
  ...
  const id = uuidv4();
  db.prepare(`INSERT INTO conversations (id, user1_id, user2_id) VALUES (?, ?, ?)`).run(id, uid, targetUserId);
